...
var BMCP_PROTOCOL_MAGIC = 1112359760;
var fetchBitcoinTransaction = (sendRequester, config, txid) => {
...
async function getBitcoinTransaction(runtime2, txid) {
  runtime2.log(`Fetching Bitcoin transaction: ${txid}`);
  const httpCapability = new cre.capabilities.HTTPClient;
  const tx = httpCapability.sendRequest(runtime2, (sendRequester, config) => {
  ...
  runtime2.log(`✅ Transaction fetched successfully!`);
  if (tx.confirmations) {
    runtime2.log(`   Confirmations: ${tx.confirmations}`);
  } else {
    runtime2.log(`   Status: Unconfirmed (in mempool)`);
  }
  return tx;
}
function extractOPReturnsFromTx(runtime2, tx) {
  runtime2.log("");
  runtime2.log("\uD83D\uDD0D Extracting OP_RETURN outputs...");
  const opReturns = [];
  for (let i2 = 0;i2 < tx.vout.length; i2++) {
    const output = tx.vout[i2];
    if (output.scriptPubKey.type === "nulldata") {
      ...
      if (pushOpcode === 76) {
        offset = 3;
      } else if (pushOpcode === 77) {
        offset = 4;
      } else if (pushOpcode === 78) {
        offset = 6;
      } else {
        offset = 2;
      }
      const data = script.slice(offset);
      opReturns.push({ index: i2, data });
    }
  }
  runtime2.log(`\uD83D\uDCE6 Found ${opReturns.length} OP_RETURN output(s)`);
  return opReturns;
}
function checkBMCPMagicInline(runtime2, data) {
  let isBMCP = false;
  try {
    if (data.length >= 4) {
...
function decodeBMCPMessageInline(runtime2, opReturnData) {
  let offset = 0;
  const protocolMagic = opReturnData.readUInt32BE(offset);
  offset += 4;
  const protocol = Buffer.from([
    protocolMagic >> 24 & 255,
    protocolMagic >> 16 & 255,
    protocolMagic >> 8 & 255,
    protocolMagic & 255
  ]).toString("ascii");
  const version2 = opReturnData[offset];
  offset += 1;
  ...
  runtime2.log(`\uD83D\uDCCB Decoded BMCP Message:`);
  runtime2.log(`   Protocol: ${decoded.protocol}`);
  runtime2.log(`   Version: ${decoded.version}`);
  runtime2.log(`   Chain Selector: 0x${decoded.chainSelector.toString(16)}`);
  runtime2.log(`   Contract: ${decoded.contract}`);
  runtime2.log(`   Nonce: ${decoded.nonce || 0}`);
  runtime2.log(`   Deadline: ${decoded.deadline || "N/A"}`);
  runtime2.log(`   Calldata Length: ${calldata.length - 2} bytes`);
  return decoded;
}
function decodeFunctionCallInline(runtime2, calldata) {
  ...
  runtime2.log(`\uD83D\uDD0D Function Selector: ${selector}`);
  const funcInfo = knownFunctions[selector];
  if (funcInfo) {
    runtime2.log(`   Signature: ${funcInfo.sig}`);
    runtime2.log(`   Status: ✅ Known function`);
  } else {
    runtime2.log(`   Signature: Unknown`);
    runtime2.log(`   Status: ⚠️  Function signature not in database`);
  }
}
function validateForExecutionInline(runtime2, decoded) {
  const errors2 = [];
  if (!decoded.contract.match(/^0x[a-fA-F0-9]{40}$/)) {
    errors2.push("Invalid target address");
  }
  if (!decoded.data || decoded.data === "0x" || decoded.data.length < 10) {
    errors2.push("No calldata provided");
  }
  const now = Math.floor(Date.now() / 1000);
  const deadlineValid = decoded.deadline && decoded.deadline > now;
  runtime2.log(`\uD83D\uDD10 Validation Checks:`);
  runtime2.log(`   Contract Address: ${errors2.includes("Invalid target address") ? "❌ Invalid" : "✅ Valid"}`);
  runtime2.log(`   Calldata Present: ${errors2.includes("No calldata provided") ? "❌ No" : "✅ Yes"}`);
  runtime2.log(`   Deadline Valid: ${deadlineValid ? "✅ Not expired" : "❌ Expired or missing"}`);
  if (errors2.length === 0) {
    runtime2.log(`✅ Message is VALID and ready for execution!`);
    runtime2.log(`\uD83D\uDE80 Ready to Execute:`);
    runtime2.log(`   • Contract: ${decoded.contract}`);
    runtime2.log(`   • Calldata: ${decoded.data.slice(0, 66)}...`);
  } else {
    runtime2.log(`❌ Message is INVALID:`);
    errors2.forEach((err) => runtime2.log(`   • ${err}`));
    if (!deadlineValid) {
      runtime2.log(`   • Deadline expired or missing`);
    }
  }
}
var onCronTrigger = async (runtime2) => {
  const exampleTxid = "967c5898bb81f7780bdde68e6d83c0903095e5650ad6fa5e76cf6cc5926947dd";
  const tx = await getBitcoinTransaction(runtime2, exampleTxid);
  const opReturns = extractOPReturnsFromTx(runtime2, tx);
  if (opReturns.length === 0) {
    runtime2.log("❌ No OP_RETURN outputs found in this transaction.");
...
\uD83D\uDD04 Processing OP_RETURN output #${index}...`);
    const isBMCP = checkBMCPMagicInline(runtime2, data);
    if (!isBMCP) {
      runtime2.log("⏩ Skipping non-BMCP message");
      continue;
    }
    runtime2.log(`✅ BMCP Message Detected in OP_RETURN output #${index}`);
    const decoded = decodeBMCPMessageInline(runtime2, data);
    lastDecoded = decoded;
    decodeFunctionCallInline(runtime2, decoded.data);
    validateForExecutionInline(runtime2, decoded);
  }
  const payload = {
  ...
  relayToContract(runtime2, payload);
  return { stringPayload: "" };
};
var relayToContract = (runtime2, payload) => {
  ...
  runtime2.log(`Write report transaction succeeded at txHash: ${txHashHex}`);
  try {
    const receipt = evmClient.getTransactionReceipt(runtime2, {
      hash: txHashHex
    }).result();
    runtime2.log(`\uD83D\uDCCB Transaction receipt retrieved for txHash: ${txHashHex}`);
    const receiptAny = receipt;
    if (receiptAny.status !== undefined) {
      if (receiptAny.status === 0) {
        runtime2.log(`⚠️  Transaction reverted at txHash: ${txHashHex}`);
        let revertReason = "Unknown revert reason";
        if (receiptAny.logs && Array.isArray(receiptAny.logs)) {
          runtime2.log(`   Logs count: ${receiptAny.logs.length}`);
        }
        if (receiptAny.errorMessage) {
          revertReason = receiptAny.errorMessage;
        } else if (receiptAny.returnData) {
          revertReason = `Return data: ${receiptAny.returnData}`;
        }
        throw new Error(`Contract execution reverted at txHash: ${txHashHex}. Reason: ${revertReason}`);
      } else if (receiptAny.status === 1) {
        runtime2.log(`✅ Transaction executed successfully (status: ${receiptAny.status})`);
        if (receiptAny.blockNumber !== undefined) {
          runtime2.log(`   Block number: ${receiptAny.blockNumber}`);
        }
        if (receiptAny.gasUsed !== undefined) {
          runtime2.log(`   Gas used: ${receiptAny.gasUsed}`);
        }
      }
    } else {
      runtime2.log(`⚠️  Transaction receipt retrieved but status field not available`);
      runtime2.log(`   Receipt keys: ${Object.keys(receiptAny).join(", ")}`);
    }
  } catch (error) {
    const errorMsg = error.message || String(error);
    if (errorMsg.includes("not found") || errorMsg.includes("Not Found")) {
      runtime2.log(`ℹ️  Transaction receipt not yet available (transaction is pending/mining)`);
      runtime2.log(`   This is expected - the transaction was successfully submitted`);
      runtime2.log(`   Check transaction status on block explorer: ${txHashHex}`);
      runtime2.log(`   Note: Contract execution revert status will only be available after the transaction is mined`);
    } else {
      runtime2.log(`⚠️  Could not fetch transaction receipt: ${errorMsg}`);
      runtime2.log(`   Transaction may still be pending. Check manually at txHash: ${txHashHex}`);
    }
  }
};
