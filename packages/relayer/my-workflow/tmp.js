...
var fetchStringInfo = (sendRequester, config) => {
...
var onCronTrigger = async (runtime2) => {
  ...
  const stringInfo = httpCapability.sendRequest(runtime2, fetchStringInfo, ConsensusAggregationByFields({
    stringPayload: identical
  }))(runtime2.config).result();
  console.log(`string payload: ${stringInfo.stringPayload}`);
  var evmConfig = "";
  var network248 = "";
  const strings = runtime2.config.strings;
  for (const string of strings) {
    if (string === stringInfo.stringPayload) {
      evmConfig = runtime2.config.evms[0];
  ...
  const encodedString = encodeAbiParameters([{ type: "string" }], [message]);
